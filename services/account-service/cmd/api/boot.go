...
import (
	...
	pkgsmtp "dietician.local/packages/smtp"
	"dietician.local/packages/tokenizer"
	"dietician.local/services/account-service/config"
)

func boot(logger *logrus.Logger, cfg *config.AccountAppScheme) (*application, error) {
	...
	smtpSender := pkgsmtp.NewSender(pkgsmtp.Config(cfg.SMTP))
...
func initConfig() (*config.AccountAppScheme, error) {
	return config.Load()
}

func initLogger(cfg *config.AccountAppScheme) *logrus.Logger {
...
func initLocalizer(cfg *config.AccountAppScheme) *i18n.Bundle {
...
func initPostgres(cfg *config.AccountAppScheme, logger *logrus.Logger) (*sqlx.DB, error) {
...
func initRedis(cfg *config.AccountAppScheme, logger *logrus.Logger) *redis.Client {
