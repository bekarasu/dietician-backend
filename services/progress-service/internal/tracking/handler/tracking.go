...
func (h *trackingHandler) CreateTrackingMetric(c *fiber.Ctx) error {
	...
	if err != nil {
		return response.Error(c, fiber.StatusInternalServerError, "failed to create tracking metric")
	}
...
func (h *trackingHandler) GetTrackingMetrics(c *fiber.Ctx) error {
	...
	if err != nil {
		return response.Error(c, fiber.StatusInternalServerError, "failed to get tracking metrics")
	}
